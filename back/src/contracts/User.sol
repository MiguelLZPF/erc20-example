...
import {
  ITokenInUser as IToken,
  IPAAPinUser as IPAAP,
  ILoanInUser as ILoan
} from "./Interfaces.sol";
import { Investment } from "./Structs.sol";

// SPDX-License-Identifier: UNLICENSED
/**
 * @title User Contract for PAAP project
 * @author Miguel Gómez Carpena, Izertis Blockchain
 * @dev User contract. All User related logic should be here.
 *      Manages the creation of a User, their properties and
 *      the record of their Loans and Investments
 */
contract User is Ownable {
  ...
  string password;
  // user's rol: "borrower" or "investor"
  string rol = "undefined";
  // user's creation date (Unix format)
  uint256 creationDate;
  /**
   * @dev Loan List. List defined by 3 parameters:
   *       - Mapping for relational search. Contract address -> Loan
   *       - Array of addresses for getting all the Loan address
   */
  address[] loans;
  mapping(address => uint256) indexLoans;
  //mapping(address => boolean) validLoan; DOES NOT MAKE SENSE
  // array that record the investments made by this user
